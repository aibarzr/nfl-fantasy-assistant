...
source manifest -> raw snapshot -> normalized/curated tables
                ...
                -> projections/prepared player values -> published dataset version
```
...
- `players`: internal identity and stable player attributes.
- `player_external_ids`: provider, external ID, internal ID, resolution method, provenance, validity/conflict state.
...
Maintain a many-provider-to-one-internal mapping with unique `(provider, external_id)` keys. Prefer exact provider IDs and authoritative crosswalks, using `gsis_id` as the preferred NFL anchor when present while retaining the internal ID for rookies and incomplete coverage.
...
Initial weekly semantic measures may include snap/target/rush/red-zone share, routes/participation, expected and actual fantasy points, points over expected, carries, targets, receptions, air yards, yards, expected touchdowns, EPA, and success rate.
