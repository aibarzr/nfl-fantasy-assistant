...
| Required promotion evidence | Current terms/rate guidance; exact endpoints/fields; supported draft semantics; complete-snapshot behavior; mapping coverage; adapter host/manifest permissions; locally pinned data/model versions; and a ready runtime dataset and identity mapping. Recommendation-runtime activation, polling/backoff, and end-to-end live-draft acceptance remain separate promotion evidence. |
